...
public class SalesAdapter extends RecyclerView.Adapter<SalesAdapter.ItemViewHolder> {
    private List<OrderListResponse.Datum> itemList;
    Context mCtx;
    String activity = "";
    private SalesAdapter.OnItemClickListener listener;
    ...
    public SalesAdapter(Context mContext, List<OrderListResponse.Datum> itemList,
    ...
    @Override
    public SalesAdapter.ItemViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_sales_list, parent, false);
        return new SalesAdapter.ItemViewHolder(view);
    }

    @Override
    public void onBindViewHolder(SalesAdapter.ItemViewHolder holder, int position) {
        OrderListResponse.Datum data = itemList.get(position);
